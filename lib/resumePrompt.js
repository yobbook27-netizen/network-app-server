...
const SHARED_STANDARD = `You are a resume writer for college students and early-career professionals.
...
/*
  ── AND IT IS CACHED, WHICH IT COULD NOT HAVE BEEN BEFORE ────────────────────────────────────

  Session 60 measured that prompt caching was useless here and was right at the time: the stable
  prefix on the largest endpoint was ~620 tokens, under the ~1,024-token minimum cacheable
  prefix, and every prompt interpolated user data from its first line.
  `cache_read_input_tokens` came back 0 on all five calls.

  BOTH HALVES OF THAT ARE NOW FALSE. The standard above is ~3,700 tokens and it is byte-identical
  on every call to either resume endpoint, so it goes in `system` rather than at the top of the
  user turn, and the user's resume follows it as the variable part. That is what makes it
  cacheable, and it is also why the two endpoints share one cache entry rather than holding one
  each.

  This is not a tidy-up. Carrying the standard cost real money the moment it was added: the same
  critique payload went from 930 input tokens to 4,617, and the call from $0.0325 to $0.1004.
  The rules are worth that; paying it again on every call is not.
*/
export const SHARED_STANDARD_SYSTEM = [
  { type: "text", text: SHARED_STANDARD, cache_control: { type: "ephemeral" } },
];
